...
|---|-----|--------|------|--------|
| 3 | **Lead with pure %-of-verified-savings** | ProsperOps vs nOps | Pure %-of-verified-savings = maximal alignment and our strongest wedge. If revenue stability is needed, add a small **fixed platform/seat fee kept separate** from the savings share — **never a percent-of-spend floor** (the "feels like a tax" churn trigger). | S |
| 4 | **EU AI Act Art. 12 framing** for the existing chain | agenticstash | Frame our append-only SHA-256 chain as the tamper-evident primitive Art. 12 automatic-logging calls for; `seal→root-digest` / `verify→pass\|fail` export surface. Framing only — we already ship a stronger `verifyChain`. Don't overclaim certified compliance. | S |
