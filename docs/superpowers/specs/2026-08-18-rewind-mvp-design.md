...
- **Packaging:** npm workspaces; `@rewind/core` and `@rewind/mcp` are separate packages, licence
  **MIT or Apache-2.0**; any future hosted/enterprise code stays in separate packages (open-core
  boundary from commit one).
