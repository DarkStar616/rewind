...
Rewind gives any coding agent **whole-workspace checkpoint and rewind** plus a **refuse-and-record
...
Rewind also ships as a Claude Code **plugin** (`packages/mcp/dist-plugin/`). Installing the plugin
...
- `scripts/guard.sh` — derives an effect key from the tool call and calls `rewind guard`.

> **Note (scoped tool names):** installed as a plugin, Rewind's own tools are exposed under scoped
> names (`mcp__rewind__checkpoint` … `mcp__rewind__guard_effect`). The `Bash|Write|Edit` matcher
> targets the agent's built-in effect tools, so the guard hook never fires on Rewind's own tools.
>
...
**One-click** — [**Add Rewind to Cursor**](cursor://anysphere.cursor-deeplink/mcp/install?name=rewind&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkByZXdpbmQvbWNwIiwibWNwIl19)

The deeplink is:

```
cursor://anysphere.cursor-deeplink/mcp/install?name=rewind&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkByZXdpbmQvbWNwIiwibWNwIl19
```

`config` is base64 of the server block `{"type":"stdio","command":"npx","args":["-y","@rewind/mcp","mcp"]}`.
