 ...
 * stdio launch (`npx -y @agent-rewind/mcp mcp`). It also pins the plugin manifest, the bundled `.mcp.json`,
...
test("plugin.json is a valid manifest named 'agent-rewind'", async () => {
  const manifest = await readJson(join(PLUGIN, ".claude-plugin", "plugin.json"));
  assert.equal(manifest.name, "agent-rewind", "plugin name must be the kebab-case id 'agent-rewind'");
...
test(".mcp.json launches the stdio server via npx", async () => {
  ...
  const server = cfg.mcpServers?.["agent-rewind"];
  assert.ok(server, ".mcp.json must define an mcpServers['agent-rewind'] entry");
  assert.equal(server.command, "npx");
  assert.deepEqual(server.args, ["-y", "@agent-rewind/mcp", "mcp"]);
});
...
test("guard.sh exists, is a shell script, and is executable", async () => {
  ...
  assert.match(body, /agent-rewind|@agent-rewind\/mcp/, "guard.sh must invoke the agent-rewind guard");
...
test("the install docs carry the Claude Code, Codex, and Cursor snippets", async () => {
  ...
  assert.match(doc, /\[mcp_servers\.agent-rewind\]/, "a TOML block for Codex CLI");
  assert.match(doc, /claude mcp add/, "the claude mcp add one-liner");
  assert.match(doc, /npx -y @agent-rewind\/mcp mcp/, "the zero-install launch command");
});
...
test("the Cursor deeplink's base64 config decodes to the exact stdio launch", async () => {
  ...
  const [, name, b64] = m;
  assert.equal(name, "agent-rewind");
  ...
  assert.deepEqual(decoded.args, ["-y", "@agent-rewind/mcp", "mcp"]);
});
